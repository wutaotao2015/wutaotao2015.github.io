...
## vmware nat correct way to config
vmware guest machine network configuration is a big problem haunting many developers
including myself, how can we make guest linux machine's ip static? Just configuring the
guest linux machine itself is not enough if we just use bridge network for guest machine,
as we the host machine like laptop can be carried around like company or home, in this
situation, the host machine's ip can be changed, like changing from 192.168.1.x to 
192.168.101.x, so in this situation, we need to use techniques to make the guest machine's
ip static in both these two environments.

I happen to search again a zhihu's post about NAT in vmware workstation configuration, 
and after searching NAT's meaning in wiki, I get to know that NAT is the ideal technique
I want for my need, it make a wide range of ips available for my guest machine, and all
of them are available to internet via the host network card, and they can be static 
regardless of the host' ip address.

The main step is configure the network editor, choose nat mode, the child network is
set to 192.168.66.0(different from host ip section which is 192.168.1.x or 192.168.101.x)
, the nat gateway address can be set to 192.168.66.2(as the 192.168.66.1 is given to 
the virtual network card such as VMnet2 whose job is to connect the guest machines to 
the host machine), then we edit the dhcp networks ip range which can be 192.168.66.3 to 
192.168.66.250(it excludes the gateway address 192.168.66.2), after this, the network
editor is done, there are two more steps we need to get job done.

1. edit the VMnet2 network adapter in windows10 host network configuration, change its
ip to static 192.168.66.1

2. edit the guest linux network configuration, make its ip static, the gateway address
is 192.168.66.2(ATTENTION!), and the DNS we set it to 8.8.8.8, and the LAST STEP, 
edit the /etc/resolv.conf file, add one line, nameserver 8.8.8.8, then, systemctl
restart network, try ping www.baidu.com, and it's all good!
