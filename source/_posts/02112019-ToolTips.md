...
character of key such as number row keys, and hold one shift and hit another
shift twice make Capslock function, and hit any shift once again cancel it.
...
PS: I think this sticky keys conflict with my big space-ctrl key, but 
big-ctrl is not stable either, so using capslock as ctrl together with 
sticky key is awesome, this needs to be tested!
