...
释放alt键时即切换(同原生功能一样)，因为listary默认也是alt + s, 所以可以将listary的alt + s
改为alt + p, 双击ctrl键也可以。

总结:
1. ctrl + space切换输入法快捷键保持不变，这点可以修改但会引起很多问题。
2. mac中command + tab键弹出列表页面，单独按alt + space可以搜索，alt + p弹出spotlight。
3. windows中alt + tab键弹出列表页面，单独按alt + s可以搜索，双击ctrl可以弹出listary, 不在
虚拟机中时windows系统可以用alt + p弹出listary, alt + space弹出switcheroo, 这样实现mac和
windows的统一。
