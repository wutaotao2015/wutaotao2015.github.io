...
## springboot整合vue
1. springboot和vue是在同一个项目的不同模块中.
2. 使用maven resource插件将前端vue的static目录和index.html文件拷贝到后端的static目录下.
然后使用命令`mvn process-resources`即可实现拷贝.
